...
def send_mailing():
    ...
    for mailing in mailings:
        # Если end_date, то завершить рассылку
        if mailing.end_date and current_datetime >= mailing.end_date:
            mailing.status = Mailing.COMPLETED
            mailing.save()
            continue  # Не отправлять, если end_date достигнут

        # Проверка, отправлять сообщение сейчас
...
def get_messages_from_cache():
    """
    Получение сообщений из кэша. Если кэш пуст,то из БД.
    """
